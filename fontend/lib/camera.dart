...
class _SelectImageScreenState extends State<SelectImageScreen> {
  ...
  Future<List<Detection>> sendToYolo(
    ...
    final request = http.MultipartRequest(
      'POST',
      Uri.parse('${ServerConfig.getRailwayUrl()}/detect'), // ใช้ Railway URL จาก ServerConfig
    );
    request.files.add(
    ...
    return (jsonData['detection_results']['detections'] as List? ?? [])
      ...
      if (!cameraStatus.isGranted) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('กรุณาอนุญาตให้เข้าถึงกล้อง'),
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) => DisplayPictureScreen(
            ...
            imagePath: photo.path, // เพิ่ม path สำหรับการบันทึก
          ),
        ),
      );
    } catch (e) {
...
class DisplayPictureScreen extends StatefulWidget {
  ...
  final String? imagePath; // เพิ่ม path สำหรับการบันทึกรูป
    ...
    try {
      ...
      if (!status.isGranted && !await Permission.storage.isGranted && !await Permission.photos.isGranted) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('กรุณาอนุญาตให้เข้าถึงที่จัดเก็บข้อมูล'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      if (Platform.isAndroid) {
        ...
        if (!dir.existsSync()) {
          ...
          if (directory != null) {
            ...
            throw Exception("ไม่สามารถเข้าถึงที่จัดเก็บข้อมูลได้");
          }
        }
      } else {
